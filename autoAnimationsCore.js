Hooks.on('init', () => {
    game.settings.register("automated-jb2a-animations-core", "tmfx", {
    ...
    /*
        game.settings.register("automated-jb2a-animations-core", "playonhit", {
        ...
        game.settings.register("automated-jb2a-animations-core", "playonDamage", {
    ...
    */
    game.settings.register("automated-jb2a-animations-core", "EnableShield", {
        name: 'Enable Shield Spell Animation',
        hint: "Shield is set to a test animation, but still looks cool. Check this to enable it",
        scope: 'world',
        type: Boolean,
        default: false,
        config: true,
    ...
    /*
        switch (game.settings.get("automated-jb2a-animations-core", "playonDamage")) {
            case (true):
...
function itemIncludes(test) {
    return (itemName?.includes(test) || itemSource?.includes(test));
}
...
//console.log(myToken);
/*
    if (game.settings.get("automated-jb2a-animations-core", "playonhit")) {
...
/*
    function itemIncludes(test) {
        if (itemIncludes(test) || itemName.includes(test)) return true;
    }
*/
/*
    function moduleIncludes(test) {
...
Hooks.on("createChatMessage", async (msg) => {
    ...
    console.log(itemId);
    myToken = canvas.tokens.placeables.find(token => token.actor.items.get(itemId) != null);
    console.log(myToken);
    myStringArray = Array.from(msg.user.targets)
    //console.log(myStringArray);
    itemName = myToken.actor.items.get(itemId).name?.toLowerCase();
    //console.log(itemName);
    itemSource = myToken.actor.items.get(itemId).data?.data?.source?.toLowerCase() ?? "";
    //console.log(itemSource);
    /*
        let itemType = myToken.actor.items.get(itemId).type?.toLowerCase();
        function itemTypeIncludes(test) {
            if (itemType.includes(test)) return true;
        }
    */
        ...
        for (var i = 0; i < arrayLength; i++) {
            ...
            switch (true) {
                case (actorIncludes("Improved Divine Smite")):
                    ...
                    if (game.settings.get("automated-jb2a-animations-core", "tmfx")) {
                        await wait(250);
                        switch (true) {
                            case (fireColor != "pass"):
...
async function CastOnSelf() {
    if (game.settings.get("automated-jb2a-animations-core", "EnableShield")) {
...
async function ExplodeOnTarget() {
    ...
    switch (true) {
        case (itemInitemIncludescludes("blue")):
            switch (true) {
                case (itemIncludes("02")):
                    type01 = "02";
                    break;
